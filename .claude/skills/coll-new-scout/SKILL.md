...
Create a new scout — a curated, living knowledge base that monitors a topic for you.
Built through a seven-phase process that produces a resource you own, understand, and
can maintain.

To import an existing scout into the collection (one built independently or moved from
elsewhere), use `/coll-scout` instead.
