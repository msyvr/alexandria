...
<div>Generated by alexandria — <a href="https://github.com/msyvr/alexandria" target="_blank" rel="noopener noreferrer">github.com/msyvr/alexandria</a></div>
</footer>
</body>
</html>
"""
...
def by_section_index(library: dict, all_items: list[dict], items_by_section: dict[str, list[dict]]) -> str:
    ...
    for major in group_order:
        ...
        blocks.append(
            ...
            f'data-major-id="{major_slug}">All {escape(major)}</button>'
            f'</div>'
...
<label for="author-filter-select">Filter by</label>
...
def by_medium_format_index(library: dict, all_items: list[dict]) -> str:
    ...
    for form in ["physical", "digital"]:
        ...
        sections.append(
            ...
            f'<button type="button" class="subsections-toggle" '
            f'aria-expanded="false" aria-controls="{panel_id}" '
            f'data-major-id="{form}">All {escape(form_cap)}</button>'
            f'</div>'
            f'<div id="{panel_id}" class="subsections-panel" hidden>'
            f'{table}</div>'
            f'</div>'
