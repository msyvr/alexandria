...
- "Open my condition treatments tracker" → run `open` on the right file
- "What's new since last week?" → scan git logs across book repos
- "What does my tracker say about treatment X?" → read the YAML, answer directly
...
│       └── README.md      ← book overview (generated by build-tracker)
```
...
the book-type skill (build-tracker).
